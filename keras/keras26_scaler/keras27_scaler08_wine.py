...
earlyStopping = EarlyStopping(monitor='val_loss',                # 무슨 값을 기준으로 할것인지
                             ...
                             patience=5,                        # 몇번을 참아줄지/ 예를들어 5면 출력값에 뒤에서 5번째 앞에 값이 제일 크다.(함수에 따라 달라짐)
                             restore_best_weights=True,
                             verbose=1)

hist = model.fit(x_train,y_train,epochs=40,batch_size=2,   # 맨 앞에 붙은 hist는 결과 값. 즉, 내용을 history에 담겠다는 의미
...
acc(정확도) : 0.6111111111111112

minmaxscaler
acc(정확도) : 0.7222222222222222

standardscaler
acc(정확도) : 0.6388888888888888
